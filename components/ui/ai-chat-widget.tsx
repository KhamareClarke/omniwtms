...
import { MessageSquare, X, Send, Bot, Loader2, Sparkles, Brain, Zap, ChevronDown, ChevronUp, Maximize2, Minimize2, RotateCcw } from 'lucide-react';
...
export function AIChatWidget() {
  ...
  const suggestions: Suggestion[] = [
    ...
    { text: "Analyze inventory trends", icon: <Sparkles className="h-4 w-4" /> },
  ...
  const typeWriterEffect = async (text: string, messageIndex: number) => {
    ...
    setMessages(prev => prev.map((msg, idx) => 
      idx === messageIndex ? { ...msg, content: text, isThinking: false } : msg
    ));
  };
                      ...
                      {message.isThinking ? (
                        ...
                        <>
                          <p className="text-sm whitespace-pre-wrap break-words">
                            {isTypingEffect && message === messages[messages.length - 1] 
                              ? currentTypingText 
                              : message.content}
                          </p>
