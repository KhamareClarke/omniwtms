...
export const WarehouseMap: React.FC<WarehouseMapProps> = ({ locations }) => {
  ...
  useEffect(() => {
    ...
    const center: [number, number] = locations.length > 0 ? getValidCoords(locations[0].coordinates) : [51.5074, -0.1278];
  ...
  useEffect(() => {
    ...
    if (locations.length > 0) {
      const bounds = L.latLngBounds(locations.map(loc => getValidCoords(loc.coordinates)));
