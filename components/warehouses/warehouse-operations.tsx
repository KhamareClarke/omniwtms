...
export function WarehouseOperations({ warehouseId }: WarehouseOperationsProps) {
  ...
  const fetchItemsForArrival = async (arrivalId: string | null) => {
    ...
    const { data: allItems, error: fetchError } = await supabase.from('truck_items').select('*').eq('truck_arrival_id', arrivalId);
    if (fetchError) { toast.error('Failed to fetch items'); setItems([]); return; }
      ...
      {workflowComplete ? (
        <Card className="text-center py-12 mx-auto max-w-xl mt-16 animate-fade-in glass-card border border-gray-200">
          ...
          <CardHeader>
            ...
            <CardTitle className="text-3xl mb-2 font-heading bg-gradient-to-r from-[#3456FF] to-[#8763FF] bg-clip-text text-transparent">Workflow Completed</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-lg mb-4 font-sans">All items have been processed and transferred to inventory</div>
            <div className="font-semibold text-green-600 mb-8 font-sans">Warehouse Operation Complete!<br/>All {items.length} items have been processed and are now available in inventory.</div>
            <Button onClick={() => {
              setCurrentStep(0);
            ...
            {steps.map((step, idx) => {
              ...
              return (
                <React.Fragment key={step}>
                  <div className={`flex flex-col items-center ${isActive ? 'text-[#3456FF]' : isCompleted ? 'text-green-500' : 'text-gray-400'}`}>
                    <div className={`rounded-full w-12 h-12 flex items-center justify-center border-2 ${
                      isActive 
            ...
            <Card className="glass-card border-gray-200 shadow-md">
              ...
              <CardContent>
                <form className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-xs font-medium mb-1 font-sans">Vehicle Registration</label>
                    <Input name="vehicle_registration" value={truckForm.vehicle_registration} onChange={handleTruckInput} autoComplete="off" className="border-gray-300 focus:border-[#3456FF] focus:ring focus:ring-[#3456FF]/10 font-sans transition-all" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium mb-1 font-sans">Customer Name</label>
                    <Input name="customer_name" value={truckForm.customer_name} onChange={handleTruckInput} autoComplete="off" className="border-gray-300 focus:border-[#3456FF] focus:ring focus:ring-[#3456FF]/10 font-sans transition-all" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium mb-1 font-sans">Driver Name</label>
                    <Input name="driver_name" value={truckForm.driver_name} onChange={handleTruckInput} autoComplete="off" className="border-gray-300 focus:border-[#3456FF] focus:ring focus:ring-[#3456FF]/10 font-sans transition-all" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium mb-1 font-sans">Vehicle Size</label>
                    <Select name="vehicle_size" value={truckForm.vehicle_size} onValueChange={val => setTruckForm({ ...truckForm, vehicle_size: val })}>
                  ...
                  <div>
                    <label className="block text-xs font-medium mb-1 font-sans">Load Type</label>
                    <Select name="load_type" value={truckForm.load_type} onValueChange={val => setTruckForm({ ...truckForm, load_type: val })}>
                      <SelectTrigger className="border-gray-300 focus:border-[#3456FF] focus:ring focus:ring-[#3456FF]/10 font-sans transition-all">
                  ...
                  <div>
                    <label className="block text-xs font-medium mb-1 font-sans">Arrival Time</label>
                    <Input name="arrival_time" type="datetime-local" value={truckForm.arrival_time} onChange={handleTruckInput} className="border-gray-300 focus:border-[#3456FF] focus:ring focus:ring-[#3456FF]/10 font-sans transition-all" />
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-xs font-medium mb-1 font-sans">Select Warehouse</label>
                    <Select name="warehouse_id" value={truckForm.warehouse_id} onValueChange={val => setTruckForm({ ...truckForm, warehouse_id: val })}>
                      <SelectTrigger className="border-gray-300 focus:border-[#3456FF] focus:ring focus:ring-[#3456FF]/10 font-sans transition-all">
                        <SelectValue placeholder="Select Warehouse" />
                      </SelectTrigger>
                      <SelectContent className="border-gray-200 shadow-lg font-sans">
                        {warehouses.map(w => <SelectItem key={w.id} value={w.id} className="font-sans focus:bg-[#3456FF]/5">{w.name}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                  <div className="md:col-span-2 flex justify-end">
                    <Button 
                      ...
                      onClick={async () => { setLoading(true); await handleRegisterArrival(); setLoading(false); }}
            ...
            <Card className="glass-card border-gray-200 shadow-md">
              ...
              <CardContent>
                <form className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end mb-4">
                  <div>
                    <label className="block text-xs font-medium mb-1 font-sans">Description</label>
                    <Select name="description" value={itemForm.description} onValueChange={val => setItemForm({ ...itemForm, description: val })}>
                      <SelectTrigger className="border-gray-300 focus:border-[#3456FF] focus:ring focus:ring-[#3456FF]/10 font-sans transition-all">
                  ...
                  <div>
                    <label className="block text-xs font-medium mb-1 font-sans">Quantity</label>
                    <Input name="quantity" type="number" min="1" value={itemForm.quantity} onChange={handleItemInput} className="border-gray-300 focus:border-[#3456FF] focus:ring focus:ring-[#3456FF]/10 font-sans transition-all" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium mb-1 font-sans">Condition</label>
                    <select name="condition" value={itemForm.condition} onChange={handleItemInput} className="border-gray-300 focus:border-[#3456FF] focus:ring focus:ring-[#3456FF]/10 font-sans transition-all rounded px-2 py-1 w-full">
                      <option value="Good">Good</option>
                      <option value="Damaged">Damaged</option>
                    </select>
                  </div>
                  <Button type="button" disabled={!canAddItem || loading} onClick={async () => { setLoading(true); await handleAddItem(); setLoading(false); }} className="w-full bg-gradient-to-r from-[#3456FF] to-[#8763FF] text-white hover:opacity-90">
                ...
                <Button className="mt-4 w-full md:w-auto bg-gradient-to-r from-[#3456FF] to-[#8763FF] text-white hover:opacity-90" disabled={!canProceedUnloading} onClick={() => setCurrentStep(2)}>
              ...
              <CardContent>
                  ...
                  <Table className="min-w-full text-sm">
                    <TableHeader>
                      <TableRow className="bg-gray-50">
                        ...
                        <TableHead className="whitespace-nowrap">Damage Image</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {items.map((item, idx) => (
                ...
                <div className="mt-4 flex flex-col md:flex-row gap-2 items-center justify-end">
                  ...
                  <Button 
                    ...
                    onClick={async () => { setCompleting(true); await handleFinishQualityCheck(); setCompleting(false); }} 
                ...
                <div className="overflow-x-auto rounded-lg border border-gray-200">
                  <Table className="min-w-full text-sm">
                    <TableHeader>
                      <TableRow className="bg-gray-50">
                        <TableHead className="whitespace-nowrap">Description</TableHead>
                        <TableHead className="whitespace-nowrap">Aisle</TableHead>
                      ...
                      {items.flatMap((item) =>
                        Array.from({ length: item.quantity }, (_, idx) => {
                          ...
                          return (
                            <TableRow key={rowKey}>
                              <TableCell className="whitespace-nowrap">{item.description}</TableCell>
                              <TableCell><Input value={putawayForm[rowKey]?.aisle || ''} onChange={e => handlePutawayInput(rowKey, 'aisle', e.target.value)} className="border-gray-300 focus:border-[#3456FF] focus:ring focus:ring-[#3456FF]/10 font-sans transition-all" /></TableCell>
                              <TableCell><Input value={putawayForm[rowKey]?.bay || ''} onChange={e => handlePutawayInput(rowKey, 'bay', e.target.value)} className="border-gray-300 focus:border-[#3456FF] focus:ring focus:ring-[#3456FF]/10 font-sans transition-all" /></TableCell>
                              <TableCell><Input value={putawayForm[rowKey]?.level || ''} onChange={e => handlePutawayInput(rowKey, 'level', e.target.value)} className="border-gray-300 focus:border-[#3456FF] focus:ring focus:ring-[#3456FF]/10 font-sans transition-all" /></TableCell>
                              <TableCell><Input value={putawayForm[rowKey]?.position || ''} onChange={e => handlePutawayInput(rowKey, 'position', e.target.value)} className="border-gray-300 focus:border-[#3456FF] focus:ring focus:ring-[#3456FF]/10 font-sans transition-all" /></TableCell>
                            </TableRow>
                          );
                        })
                      )}
                  ...
                  {items.flatMap((item) =>
                    Array.from({ length: item.quantity }, (_, idx) => {
                      ...
                      if (truckForm.arrival_time) {
                        ...
                        arrivalTime = d.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                      }
                      return (
                        <div key={rowKey} className="flex flex-col items-center mb-8 relative group">
                          <div className="absolute top-0 right-0 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => {
                                const labelElement = document.getElementById(`label-${rowKey}`);
                                if (labelElement) {
                          ...
                          <div id={`label-${rowKey}`} className="transform hover:scale-105 transition-transform duration-200">
                            <WarehouseLabel
                              company={truckForm.customer_name ? truckForm.customer_name + ' Warehouse' : 'Warehouse Label'}
