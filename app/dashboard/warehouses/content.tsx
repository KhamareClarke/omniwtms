...
export default function WarehousesPage() {
  ...
  const [editingLocationWarehouse, setEditingLocationWarehouse] = useState<Warehouse | null>(null);
  ...
  const fetchMovements = async () => {
    try {
      ...
      if (data && data.length > 0) {
        const { data: movementsWithDetails, error: detailsError } = await supabase
          .from('inventory_movements')
          .select(`
            *,
  ...
  const handleAssignStock = async (quantity: number) => {
    ...
    try {
      ...
      const movementData = {
        ...
        notes: `Assigned ${quantity} units of ${selectedProductDetails?.name || 'product'} to ${selectedWarehouseDetails?.name || 'warehouse'}`,
  ...
  const handleUpdateWarehouseLocation = async () => {
    ...
    try {
      ...
      const coords = [parseFloat(locationData[0].lat), parseFloat(locationData[0].lon)];
  ...
  return (
    <AnimatedGradientBackground className="min-h-screen">
      ...
      <div className="w-full mb-6">
        <div className="rounded-xl overflow-hidden border border-gray-200 shadow-md bg-white/80 backdrop-blur-sm" style={{ minHeight: '250px', height: '40vw', maxHeight: 400 }}>
        ...
        {validLocations.length === 0 && (
          <div className="mt-2 text-sm text-yellow-700 bg-yellow-100 border border-yellow-300 rounded-lg p-2">
            <strong>No warehouses with valid coordinates to show on the map.</strong><br />
      ...
      <div className="container mx-auto p-2 sm:p-4 space-y-4 sm:space-y-6 relative min-h-screen">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-500 mt-1 font-sans text-sm sm:text-base">Manage your warehouse locations and inventory</p>
          </div>
                    ...
                    <TableRow key={warehouse.id} className="hover:bg-gray-50/70 backdrop-blur-sm transition-colors group">
                      ...
                      >
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487a5.25 5.25 0 11-9.724 0M12 2.25v9.75m0 0c-3.728 0-6.75 2.343-6.75 5.25v.75a.75.75 0 00.75.75h12a.75.75 0 00.75-.75v-.75c0-2.907-3.022-5.25-6.75-5.25z" />
                        </svg>
                      </Button>
                  </TableCell>
                </TableRow>
      ...
      <Dialog open={showAssignDialog} onOpenChange={setShowAssignDialog}>
        ...
        <Dialog open={!!editingLocationWarehouse} onOpenChange={(open) => !open && setEditingLocationWarehouse(null)}>
          <DialogContent className="sm:max-w-[400px] glass-card border-gray-200">
            <DialogHeader>
              <DialogTitle className="text-lg font-heading font-bold">Edit Warehouse Location</DialogTitle>
            </DialogHeader>
