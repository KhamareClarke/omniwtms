...
export default function CouriersPage() {
  ...
  const handleDeleteCourier = async (courierId: string) => {
    ...
    try {
      ...
      // Update local state
      setCouriers(prevCouriers => prevCouriers.filter(c => c.id !== courierId));
  ...
  return (
    <div className="container mx-auto p-2 sm:p-4 space-y-4 sm:space-y-6">
              ...
              {isTableLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={8} className="text-center py-8">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p>No couriers found. Add your first courier using the button above.</p>
                    </div>
                  </TableCell>
                </TableRow>
              ) : (
                couriers.map((courier) => (
                  <TableRow key={courier.id}>
                    <TableCell className="whitespace-nowrap">{courier.name}</TableCell>
                    <TableCell className="whitespace-nowrap">{courier.email}</TableCell>
                    <TableCell className="whitespace-nowrap">{courier.phone}</TableCell>
                    <TableCell className="whitespace-nowrap">
                      <div className="flex flex-col">
                        <span>{courier.vehicle_type}</span>
                        <span className="text-sm text-gray-500">{courier.vehicle_registration}</span>
                      </div>
                    </TableCell>
                    <TableCell className="whitespace-nowrap">{courier.assigned_region}</TableCell>
                    <TableCell className="whitespace-nowrap">
                      <Badge variant={
                        courier.status === 'active' ? 'success' : 
                        courier.status === 'delayed' ? 'warning' : 'secondary'
                      } className={
                        courier.status === 'active' ? 'bg-green-100 text-green-800 border-green-200 font-medium' :
                        courier.status === 'delayed' ? 'bg-yellow-100 text-yellow-800 border-yellow-200 font-medium' :
                        'bg-gray-100 text-gray-800 border-gray-200 font-medium'
                      }>
                        {courier.status.charAt(0).toUpperCase() + courier.status.slice(1)}
                      </Badge>
                    </TableCell>
                    <TableCell className="whitespace-nowrap">{courier.deliveries_completed}</TableCell>
                    <TableCell className="whitespace-nowrap">
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="whitespace-nowrap">Shipping Label</TableHead>
                  <TableHead className="whitespace-nowrap">Route Details</TableHead>
                ...
                {isDeliveriesLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={10} className="text-center py-8">
                      <div className="flex flex-col items-center gap-2">
                        <Package className="h-8 w-8 text-gray-400" />
                        <p>No deliveries found. Assign your first delivery using the button above.</p>
                      </div>
                    </TableCell>
                  </TableRow>
                ) : (
                  deliveries.map((delivery) => (
                    <TableRow key={delivery.id}>
                      <TableCell className="whitespace-nowrap">{delivery.package_id}</TableCell>
                      <TableCell className="whitespace-nowrap">
                        <div className="flex flex-col">
                          <span>{delivery.courier?.name}</span>
                          <span className="text-sm text-gray-500">{delivery.courier?.vehicle_type}</span>
                        </div>
                      </TableCell>
                      <TableCell className="whitespace-nowrap">
                        <Badge variant={
                          ...
                          delivery.priority === 'high' ? 'bg-red-100 text-red-800 border-red-200 font-medium' :
                          delivery.priority === 'medium' ? 'bg-yellow-100 text-yellow-800 border-yellow-200 font-medium' :
                          'bg-gray-100 text-gray-800 border-gray-200 font-medium'
                        }>
                      ...
                      <TableCell className="whitespace-nowrap">
                        <Badge variant={
                          ...
                          delivery.status === 'completed' ? 'bg-green-100 text-green-800 border-green-200 font-medium' :
                          delivery.status === 'in_progress' ? 'bg-yellow-100 text-yellow-800 border-yellow-200 font-medium' :
                          delivery.status === 'failed' ? 'bg-red-100 text-red-800 border-red-200 font-medium' :
                          'bg-gray-100 text-gray-800 border-gray-200 font-medium'
                        }>
                      ...
                      <TableCell className="whitespace-nowrap">
                        {delivery.status !== 'completed' && (
                          <div className="flex flex-wrap gap-2">
                            ...
                            {delivery.status === 'in_progress' && !delivery.pod_file && (
                              <Label htmlFor={`pod-upload-${delivery.id}`} className="cursor-pointer">
                                <Input
                                ...
                                />
                                <Badge variant="outline" className="bg-yellow-100 hover:bg-yellow-200 text-yellow-700 border-yellow-200 cursor-pointer font-medium">
                                  Upload POD
                                </Badge>
                              </Label>
                            )}
                              ...
                              >
                                ...
                                <div className="text-xs space-y-1">
                                  ...
                                  <div className="space-y-1 ml-4">
                                    {route.stops.map((stop: any, currentIndex: number) => (
                                      <div key={currentIndex} className="flex items-start gap-3 p-2 border rounded-lg">
                                        <div className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0
                                          ${currentIndex === 0 ? 'bg-blue-100 text-blue-600' : 
                                            currentIndex === route.stops.length - 1 ? 'bg-green-100 text-green-600' : 
                                        ...
                                        <div className="flex-1 truncate">
                                          {stop.name}
                                          {currentIndex === 0 && <span className="text-blue-600 ml-1">(Start)</span>}
                                          {currentIndex === route.stops.length - 1 && <span className="text-green-600 ml-1">(End)</span>}
                                        </div>
                                      </div>
                                    ))}
                                  </div>
            ...
            <div className="space-y-4">
              <div className="border rounded-lg p-4 space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-bold text-lg">Package ID: {selectedLabel.packageId}</h3>
                    <p className="text-sm text-gray-500">Created: {new Date(selectedLabel.createdAt).toLocaleString()}</p>
                  </div>
                  <Badge variant={selectedLabel.status === 'pending' ? 'warning' : 'success'}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-gray-500">Time: {selectedLabel.pickup.time}</p>
                  </div>
                  <div className="space-y-2">
                    <h4 className="font-semibold">Delivery Details</h4>
                    <p>{selectedLabel.delivery.address}</p>
                    {selectedLabel.delivery.notes && (
                      <p className="text-sm text-gray-500">Notes: {selectedLabel.delivery.notes}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center pt-2 border-t">
                  <div>
                    ...
                    <p className="font-semibold">{selectedLabel.totalWeight} kg</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Priority</p>
                    <Badge variant={
                      selectedLabel.priority === 'high' ? 'destructive' :
                      selectedLabel.priority === 'medium' ? 'warning' : 'default'
                    }>
      ...
      <Dialog open={showRouteDetailsDialog} onOpenChange={setShowRouteDetailsDialog}>
          ...
          {selectedRouteDetails && (
            <div className="space-y-4">
              <div className="border rounded-lg p-4 space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-bold text-lg">{selectedRouteDetails.name}</h3>
                    <p className="text-sm text-gray-500">{selectedRouteDetails.description}</p>
                  </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-gray-50 p-4 rounded-lg">
                  <div>
                    ...
                    <p className="font-semibold">{selectedRouteDetails.distance.toFixed(1)} km</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Duration</p>
                    <p className="font-semibold">{selectedRouteDetails.duration.toFixed(0)} min</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-semibold">₨{selectedRouteDetails.totalFuelCost.toFixed(0)}</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {selectedRouteDetails.stops.map((stop: any, currentIndex: number) => (
                      <div key={currentIndex} className="flex items-start gap-3 p-2 border rounded-lg">
                        <div className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0
                          ${currentIndex === 0 ? 'bg-blue-100 text-blue-600' : 
                            currentIndex === selectedRouteDetails.stops.length - 1 ? 'bg-green-100 text-green-600' : 
                            'bg-gray-100 text-gray-600'}`}
                        >
                          {currentIndex + 1}
                        </div>
                        <div className="flex-1 truncate">
                          {stop.name}
                          {currentIndex === 0 && <span className="text-blue-600 ml-1">(Start)</span>}
                          {currentIndex === selectedRouteDetails.stops.length - 1 && <span className="text-green-600 ml-1">(End)</span>}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {Array.isArray(selectedRouteDetails.advantages) && selectedRouteDetails.advantages.length > 0 ? (
                      selectedRouteDetails.advantages.map((advantage: string, index: number) => (
                        <Badge key={index} variant="outline" className="bg-green-50">
                          {advantage}
                        </Badge>
                      ))
                    ) : (
                      <span className="text-gray-400 text-sm">No advantages listed.</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          <form onSubmit={handleUpdateCourier} className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="edit-vehicle_registration">Registration</Label>
                  <Input
                ...
                <div className="space-y-2">
                  <Label htmlFor="edit-max_capacity">Max Capacity (packages)</Label>
                  <Input
