...
export default function ReportsPage() {
  ...
  const supabase = createClientComponentClient({
    ...
    supabaseKey: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFwa2FrbG1iaXdpdGxyb3lramltIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzY4MTM4NjIsImV4cCI6MjA1MjM4OTg2Mn0.4y_ogmlsnMMXCaISQeVo-oS6zDJnyAVEeAo6p7Ms97U'
  });
    ...
    try {
      ...
      const transformedData = [
        ...
        ...warehouses.flatMap((w: Warehouse) => (w.inventory_movements || []).map((m: InventoryMovement) => ({
          ...
          details: `Warehouse: ${w.name || 'Unknown'}, Type: ${m.movement_type}, Quantity: ${m.quantity}`,
          status: 'success'
        })))
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="font-medium text-white">{stats.products.total}</span>
                </div>
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-4 text-sm">
                <div className="bg-white/5 rounded-lg p-2">
                  ...
                  <p className="font-medium text-white">{stats.products.addedToday}</p>
                </div>
                <div className="bg-white/5 rounded-lg p-2">
                  <p className="text-gray-400">This Week</p>
                  <p className="font-medium text-white">{stats.products.addedThisWeek}</p>
                </div>
                <div className="bg-white/5 rounded-lg p-2">
                  <p className="text-gray-400">This Month</p>
                  <p className="font-medium text-white">{stats.products.addedThisMonth}</p>
                </div>
              </div>
            </div>
          </CardContent>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-2 sm:gap-4">
                <div className="bg-white/5 rounded-lg p-2">
                  ...
                  <p className="font-medium text-white text-base sm:text-lg">{stats.warehouse.total}</p>
                </div>
                <div className="bg-white/5 rounded-lg p-2">
                  <p className="text-gray-400 text-sm">Total Stocks</p>
                  <p className="font-medium text-white text-base sm:text-lg">{stats.warehouse.totalStocks}</p>
                </div>
              </div>
              <div>
                ...
                <div className="space-y-2">
                  {stats.warehouse.utilization.map((w, i) => (
                    <div key={i} className="mb-2">
                      <div className="flex justify-between text-sm mb-1">
                        <span className="truncate mr-2 text-gray-400">{w.name}</span>
                        <span className="whitespace-nowrap text-white">{Math.round(w.utilization)}%</span>
                      </div>
                      <Progress value={w.utilization} className="bg-indigo-200/20 h-2">
                        <div 
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-2 sm:gap-4">
                <div className="bg-white/5 rounded-lg p-2">
                  ...
                  <p className="font-medium text-white text-base sm:text-lg">{stats.deliveries.total}</p>
                </div>
                <div className="bg-white/5 rounded-lg p-2">
                  <p className="text-gray-400 text-sm">Completed</p>
                  <p className="font-medium text-white text-base sm:text-lg">{stats.deliveries.completed}</p>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <div className="bg-white/5 rounded-lg p-2">
                  <p className="text-gray-400 text-sm">In Progress</p>
                  <p className="font-medium text-white text-sm sm:text-base">{stats.deliveries.inProgress}</p>
                </div>
                <div className="bg-white/5 rounded-lg p-2">
                  <p className="text-gray-400 text-sm">Failed</p>
                  <p className="font-medium text-white text-sm sm:text-base">{stats.deliveries.failed}</p>
                </div>
                <div className="bg-white/5 rounded-lg p-2">
                  <p className="text-gray-400 text-sm">Pending</p>
                  <p className="font-medium text-white text-sm sm:text-base">{stats.deliveries.pending}</p>
                </div>
              </div>
              <div>
        ...
        <CardContent className="p-4 sm:p-6">
          <div className="space-y-4">
            {stats.timeline.map((activity, i) => (
              <div key={i} className="flex flex-col sm:flex-row sm:gap-4 items-start bg-white/5 rounded-lg p-3 sm:p-4">
                <div className="w-full sm:w-32 text-sm text-gray-500 mb-2 sm:mb-0">
                  {format(new Date(activity.date), 'MMM dd, HH:mm')}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-medium truncate text-white">{activity.activity}</p>
                  <p className="text-sm text-gray-400 truncate">{activity.details}</p>
                </div>
                <div className={`px-3 py-1.5 rounded-full text-xs font-medium mt-2 sm:mt-0 ${
