...
import { Warehouse, TruckIcon, BoxIcon, BarChart3Icon, Users2Icon, LayoutDashboard, LockIcon, BarChart, Zap, Shield, Globe, Database, CheckCircle, ArrowRight } from 'lucide-react';
  ...
  <div className="absolute inset-0 -z-10 overflow-hidden">
    <div className="absolute -z-10 h-full w-full bg-gradient-to-br from-slate-50 to-white">
      ...
      <div className="absolute inset-0 -z-5 opacity-20">
        <div className="absolute top-[10%] left-[15%] h-1.5 w-1.5 rounded-full bg-blue-600 animate-ping" style={{ animationDuration: '3s', animationDelay: '0.5s' }} />
        <div className="absolute top-[65%] left-[30%] h-1.5 w-1.5 rounded-full bg-cyan-500 animate-ping" style={{ animationDuration: '5s', animationDelay: '2s' }} />
        <div className="absolute top-[80%] left-[70%] h-1 w-1 rounded-full bg-pink-500 animate-ping" style={{ animationDuration: '4.5s', animationDelay: '2.5s' }} />
      </div>
...
const FeatureCard = ({ icon: Icon, title, description }: { icon: any, title: string, description: string }) => (
  <motion.div
    ...
    whileHover={{ scale: 1.03, boxShadow: "0 15px 30px -10px rgba(0, 0, 0, 0.15)" }}
  ...
  >
    ...
    <div className="relative z-10">
      <div className="flex items-center mb-4">
        ...
        <h3 className="text-xl font-bold text-gray-800 bg-gradient-to-br from-gray-800 to-gray-600 bg-clip-text text-transparent">{title}</h3>
      </div>
...
export default function LoginPage() {
  ...
  const triggerAutoFill = () => {
    ...
    setTimeout(() => {
      if (activeTab === 'client') {
        setClientFormData({ email: 'demo@omnideploy.ai', password: 'AIpowered2023' });
      } else if (activeTab === 'courier') {
        setCourierFormData({ email: 'driver@omnideploy.ai', password: 'AIpowered2023' });
      } else {
        setCustomerFormData({ email: 'customer@omnideploy.ai', password: 'AIpowered2023' });
      }
    }, 600);
  ...
  const handleClientSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Set success state before redirect to show loading indicator
  ...
  const handleCourierSignIn = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Attempting courier login with:', courierFormData);
      
      // First try to find the courier
      const { data: existingCourier, error: findError } = await supabase
        .from('couriers')
        .select('*')
        .eq('email', courierFormData.email)
        .single();

      // If courier doesn't exist, create one
      if (!existingCourier) {
          ...
          .insert([
            {
              ...
              name: courierFormData.email.split('@')[0],
              phone: '+1234567890',
              vehicle_type: 'Car',
              vehicle_registration: 'TMP123',
              status: 'active',
        ...
        if (createError) {
          console.error('Error creating courier:', createError);
          throw new Error('Failed to create courier account');
        }

        // Store the new courier data
        localStorage.setItem('currentCourier', JSON.stringify(newCourier));
        toast.success(`Welcome, ${newCourier.name}!`);
        setLoginSuccess(true);
        setTimeout(() => {
          router.push('/courier');
        }, 500);
      ...
      // Verify password for existing courier
      if (existingCourier.password !== courierFormData.password) {
        console.error('Password mismatch');
        throw new Error('Invalid credentials');
      }

      // Store courier data in localStorage
      localStorage.setItem('currentCourier', JSON.stringify(existingCourier));
      toast.success(`Welcome back, ${existingCourier.name}!`);
    ...
    <div className="min-h-screen w-full flex flex-col">
      ...
      <header className="w-full flex items-center justify-between px-4 md:px-8 py-4 backdrop-blur-md bg-white/60 sticky top-0 z-50 border-b border-white/20">
        ...
        <nav className="hidden md:flex gap-6 items-center text-gray-600 text-sm">
          <Link href="#features" className="hover:text-blue-700 transition-colors flex items-center gap-1">
            <Zap className="h-3.5 w-3.5 text-yellow-500" />
            Features
          </Link>
          <Link href="#pricing" className="hover:text-blue-700 transition-colors flex items-center gap-1">
            <Database className="h-3.5 w-3.5 text-blue-500" />
            Solutions
          </Link>
          <Link href="#support" className="hover:text-blue-700 transition-colors flex items-center gap-1">
        ...
        <Button variant="outline" size="sm" className="md:hidden border border-gray-200">
          Menu
        </Button>
      </header>
          ...
          >
            ...
            <Card className="border-0 shadow-2xl bg-white/95 backdrop-blur-md rounded-xl overflow-hidden w-full">
              <CardHeader className="pb-2 text-center relative">
                ...
                <div className="mx-auto h-12 w-12 sm:h-14 sm:w-14 rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 text-white flex items-center justify-center mb-3 shadow-lg shadow-blue-500/20 relative overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-br from-blue-500/40 to-indigo-500/40 animate-pulse" style={{ animationDuration: '3s' }}></div>
                  <Warehouse className="h-6 w-6 sm:h-7 sm:w-7 relative z-10" />
                </div>
                <CardTitle className="text-xl sm:text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-br from-gray-800 to-gray-600 font-sans">AI-Powered Logistics Platform</CardTitle>
                <CardDescription className="text-gray-500 mt-1 text-sm sm:text-base">Sign in to your account</CardDescription>
              </CardHeader>
              <CardContent className="px-4 sm:px-6 py-3">
                <Tabs defaultValue="client" className="w-full" onValueChange={setActiveTab}>
                  <TabsList className="grid w-full grid-cols-3 mb-5 bg-gray-50/80 rounded-xl overflow-hidden p-1 border border-gray-100 backdrop-blur-sm">
                    <TabsTrigger value="client" className="text-xs sm:text-sm py-2 rounded-lg data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-600 data-[state=active]:via-indigo-600 data-[state=active]:to-blue-700 data-[state=active]:text-white data-[state=active]:shadow-md transition-all duration-300 font-medium">Organization</TabsTrigger>
                    <TabsTrigger value="customer" className="text-xs sm:text-sm py-2 rounded-lg data-[state=active]:bg-gradient-to-r data-[state=active]:from-purple-600 data-[state=active]:via-pink-600 data-[state=active]:to-purple-700 data-[state=active]:text-white data-[state=active]:shadow-md transition-all duration-300 font-medium">Customer</TabsTrigger>
                    <TabsTrigger value="courier" className="text-xs sm:text-sm py-2 rounded-lg data-[state=active]:bg-gradient-to-r data-[state=active]:from-green-600 data-[state=active]:via-teal-600 data-[state=active]:to-green-700 data-[state=active]:text-white data-[state=active]:shadow-md transition-all duration-300 font-medium">Courier</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="client">
                    <div className="mb-4 p-3 bg-gradient-to-r from-blue-50/50 to-indigo-50/50 border border-blue-100 rounded-lg text-blue-700 text-xs sm:text-sm flex items-start gap-2 sm:gap-3 backdrop-blur-sm">
                      ...
                      <div>
                        <div className="font-semibold mb-0.5 sm:mb-1 text-transparent bg-clip-text bg-gradient-to-r from-blue-700 to-indigo-700">Organization Portal</div>
                        <div className="text-xs text-blue-600">Access your organization's logistics dashboard with AI insights</div>
                      </div>
              </div>
                  <form onSubmit={handleClientSignIn} className="space-y-4">
                      <div className="space-y-2">
                        <Label htmlFor="client-email" className="text-xs sm:text-sm font-medium flex items-center justify-between">
                          <span className="text-transparent bg-clip-text bg-gradient-to-r from-gray-700 to-gray-500">Email</span>
                        </Label>
                      ...
                      <Input
                        ...
                        required
                            className={`bg-gray-50/80 backdrop-blur-sm border border-gray-200 pl-10 rounded-lg focus:ring-2 focus:ring-blue-500/20 transition-all text-sm ${autoFillAnimation ? 'animate-pulse bg-blue-50' : ''}`}
                          />
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="client-password" className="text-xs sm:text-sm font-medium text-transparent bg-clip-text bg-gradient-to-r from-gray-700 to-gray-500">Password</Label>
                        <div className="relative">
                      <Input
                        id="client-password"
                        ...
                        required
                            className={`bg-gray-50/80 backdrop-blur-sm border border-gray-200 pl-10 pr-10 rounded-lg focus:ring-2 focus:ring-blue-500/20 transition-all text-sm ${autoFillAnimation ? 'animate-pulse bg-blue-50' : ''}`}
                />
                          <div className="absolute left-3 top-1/2 -translate-y-1/2 text-blue-500">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          ...
                          <button 
                            type="button"
                            onClick={() => setShowPassword(prev => ({ ...prev, client: !prev.client }))}
                            className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-blue-600 transition-colors"
                            aria-label={showPassword.client ? "Hide password" : "Show password"}
                          >
                            {showPassword.client ? (
                              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      ...
                      >
                        ...
                        {isLoading ? (
                          ...
                          <span className="flex items-center justify-center gap-2 relative z-10">
                            <svg className="h-4 w-4 sm:h-5 sm:w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          ...
                          <span className="flex items-center justify-center gap-2 relative z-10">
                            Sign In <ArrowRight className="h-3.5 w-3.5 sm:h-4 sm:w-4 animate-pulse" style={{ animationDuration: '2s' }} />
                          </span>
                        )}
                </Button>
              </form>
                    <div className="mt-3 text-center text-xs sm:text-sm text-gray-500">
                      <div className="flex flex-col gap-2">
                        <a href="#" className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 hover:underline flex items-center justify-center gap-1.5 group">
                          <span>Forgot password?</span>
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 sm:h-3.5 sm:w-3.5 transform group-hover:translate-x-1 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <TabsContent value="customer">
                    <div className="mb-4 p-3 bg-gradient-to-r from-purple-50/50 to-pink-50/50 border border-purple-100 rounded-lg text-purple-700 text-xs sm:text-sm flex items-start gap-2 sm:gap-3 backdrop-blur-sm">
                      ...
                      <div>
                        <div className="font-semibold mb-0.5 sm:mb-1 text-transparent bg-clip-text bg-gradient-to-r from-purple-700 to-pink-700">Customer Portal</div>
                        <div className="text-xs text-purple-600">Track your orders and manage your shipments</div>
                      </div>
                    </div>
              <form onSubmit={handleCustomerSignIn} className="space-y-4">
                      <div className="space-y-2">
                        <Label htmlFor="customer-email" className="text-xs sm:text-sm font-medium flex items-center justify-between">
                          Email
                        </Label>
                        <div className="relative">
                <Input
                  ...
                  required
                            className={`bg-gray-50 border border-gray-200 pl-10 ${autoFillAnimation ? 'animate-pulse bg-blue-50' : ''}`}
                          />
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="customer-password" className="text-xs sm:text-sm font-medium">Password</Label>
                        <div className="relative">
                <Input
                  ...
                  required
                            className={`bg-gray-50 border border-gray-200 pl-10 pr-10 ${autoFillAnimation ? 'animate-pulse bg-blue-50' : ''}`}
                />
                          <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                          ...
                          <button 
                            type="button"
                            onClick={() => setShowPassword(prev => ({ ...prev, customer: !prev.customer }))}
                            className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-purple-600 transition-colors"
                            aria-label={showPassword.customer ? "Hide password" : "Show password"}
                          >
                    ...
                    <div className="mt-4 text-center text-sm text-gray-500">
                      <div className="flex flex-col gap-2">
                        <a href="#" className="text-purple-600 hover:underline flex items-center justify-center gap-1.5 group">
                          <span>Forgot password?</span>
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 transform group-hover:translate-x-1 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
                          </svg>
                        </a>
                        <div className="border-t border-gray-100 my-2"></div>
            ...
            <TabsContent value="courier">
                    <div className="mb-4 p-3 bg-gradient-to-r from-green-50/50 to-teal-50/50 border border-green-100 rounded-lg text-green-700 text-xs sm:text-sm flex items-start gap-2 sm:gap-3 backdrop-blur-sm">
                      ...
                      <div>
                        <div className="font-semibold mb-0.5 sm:mb-1 text-transparent bg-clip-text bg-gradient-to-r from-green-700 to-teal-700">Courier Portal</div>
                        <div className="text-xs text-green-600">Manage deliveries with AI-optimized routes</div>
                      </div>
              </div>
                  <form onSubmit={handleCourierSignIn} className="space-y-4">
                      <div className="space-y-2">
                        <Label htmlFor="courier-email" className="text-xs sm:text-sm font-medium flex items-center justify-between">
                          Email
                        </Label>
                        <div className="relative">
                      <Input
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="courier-password" className="text-xs sm:text-sm font-medium">Password</Label>
                        <div className="relative">
                      <Input
                        id="courier-password"
                      ...
                      />
                          ...
                          <button 
                            type="button"
                            onClick={() => setShowPassword(prev => ({ ...prev, courier: !prev.courier }))}
                            className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-green-600 transition-colors"
                            aria-label={showPassword.courier ? "Hide password" : "Show password"}
                          >
                    ...
                    <div className="mt-4 text-center text-sm text-gray-500">
                      <div className="flex flex-col gap-2">
                        <a href="#" className="text-green-600 hover:underline flex items-center justify-center gap-1.5 group">
                          <span>Forgot password?</span>
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 transform group-hover:translate-x-1 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
                          </svg>
              ...
              <CardFooter className="px-4 sm:px-6 py-3 bg-gray-50/50 border-t border-gray-100 rounded-b-lg">
                <div className="w-full flex items-center gap-2 text-xs text-gray-500">
                  ...
                  <span>Protected by enterprise-grade security and AI threat detection</span>
                </div>
              </CardFooter>
            </Card>
          </motion.div>
        ...
        <div className="flex-1 flex flex-col items-center lg:items-start justify-center w-full max-w-xl lg:max-w-2xl order-1 lg:order-2 mb-6 lg:mb-0 mx-auto lg:mx-0">
          ...
          >
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold bg-gradient-to-r from-[#3456FF] via-[#8763FF] to-[#FF3456] bg-clip-text text-transparent mb-3 sm:mb-4 leading-tight tracking-tight">
              Next-Gen<br className="hidden sm:block" /> Logistics
            </h1>
            <p className="text-gray-600 text-sm sm:text-base lg:text-lg max-w-lg mx-auto lg:mx-0 lg:pr-8 leading-relaxed">
              Streamline operations, boost productivity, and gain real-time visibility with predictive analytics and AI-optimized routes.
            </p>
          </motion.div>
              ...
              <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-md border border-gray-100/50 p-4 sm:p-5 relative overflow-hidden group">
                ...
                <div className="relative z-10">
                  <div className="flex items-center mb-2 sm:mb-3">
                    ...
                    <h3 className="text-base sm:text-lg font-bold text-gray-800 bg-gradient-to-br from-gray-800 to-gray-600 bg-clip-text text-transparent">AI-Powered Analytics</h3>
                  </div>
                  <p className="text-gray-600 text-xs sm:text-sm leading-relaxed">Predictive analytics and real-time dashboards with actionable insights for your logistics operations.</p>
                </div>
              </div>
            </motion.div>
              ...
              <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-md border border-gray-100/50 p-4 sm:p-5 relative overflow-hidden group">
                ...
                <div className="relative z-10">
                  <div className="flex items-center mb-2 sm:mb-3">
                    ...
                    <h3 className="text-base sm:text-lg font-bold text-gray-800 bg-gradient-to-br from-gray-800 to-gray-600 bg-clip-text text-transparent">Smart Inventory</h3>
                  </div>
                  <p className="text-gray-600 text-xs sm:text-sm leading-relaxed">Automated inventory optimization with AI demand forecasting and smart replenishment.</p>
                </div>
              </div>
            </motion.div>
              ...
              <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-md border border-gray-100/50 p-4 sm:p-5 relative overflow-hidden group">
                ...
                <div className="relative z-10">
                  <div className="flex items-center mb-2 sm:mb-3">
                    ...
                    <h3 className="text-base sm:text-lg font-bold text-gray-800 bg-gradient-to-br from-gray-800 to-gray-600 bg-clip-text text-transparent">Intelligent Routing</h3>
                  </div>
                  <p className="text-gray-600 text-xs sm:text-sm leading-relaxed">Machine learning algorithms optimize delivery routes in real-time saving time and fuel.</p>
                </div>
              </div>
            </motion.div>
              ...
              <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-md border border-gray-100/50 p-4 sm:p-5 relative overflow-hidden group">
                ...
                <div className="relative z-10">
                  <div className="flex items-center mb-2 sm:mb-3">
                    ...
                    <h3 className="text-base sm:text-lg font-bold text-gray-800 bg-gradient-to-br from-gray-800 to-gray-600 bg-clip-text text-transparent">Advanced Security</h3>
                  </div>
                  <p className="text-gray-600 text-xs sm:text-sm leading-relaxed">Enterprise-grade security with AI-powered threat detection and prevention systems.</p>
                </div>
              </div>
            </motion.div>
          </div>
          ...
          >
            <div className="text-center mb-2 sm:mb-3">
              <span className="text-xs font-semibold uppercase tracking-wider text-blue-600">TRUSTED GLOBALLY</span>
            </div>
      ...
      <footer className="w-full bg-white/80 backdrop-blur-md border-t border-gray-100 py-8 relative z-10 mt-8">
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-2">
              <div className="h-8 w-8 rounded-lg bg-gradient-to-br from-blue-600 to-indigo-600 text-white flex items-center justify-center shadow-md">
              </div>
            ...
            <div className="flex gap-8">
              <a href="#" className="text-sm text-gray-500 hover:text-transparent hover:bg-clip-text hover:bg-gradient-to-r hover:from-blue-600 hover:to-indigo-600 transition-colors relative group">
                Privacy Policy
