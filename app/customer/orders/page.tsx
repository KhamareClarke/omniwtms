          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <Dialog open={showOrderModal} onOpenChange={setShowOrderModal}>
              <DialogTrigger asChild>
                <Button className="bg-blue-600 hover:bg-blue-700 text-white w-full sm:w-auto" onClick={() => setShowOrderModal(true)}>
                  Create Order
                </Button>
              </DialogTrigger>
              <DialogContent className="max-w-lg w-full p-0">
                <div className="flex flex-col h-full max-h-[90vh]">
                  ...
                  <div className="flex gap-2 justify-end p-4 border-t bg-white sticky bottom-0 z-10">
                    <Button variant="outline" onClick={() => setShowOrderModal(false)} className="w-full sm:w-auto">Cancel</Button>
                    <Button onClick={handleCreateOrder} disabled={orderLoading || orderItems.length === 0} className="bg-blue-600 hover:bg-blue-700 text-white w-full sm:w-auto">
          ...
          <div className="bg-white rounded-xl shadow p-6">
            <div className="overflow-x-auto w-full">
              <table className="min-w-full text-xs">
                <thead>
                  <tr>
                    <th className="px-2 py-1 border-b bg-gray-50 text-left">Order ID</th>
                    ...
                    <th className="px-2 py-1 border-b bg-gray-50 text-left">Date</th>
                    <th className="px-2 py-1 border-b bg-gray-50 text-left">Status</th>
                  </tr>
                </thead>
                <tbody>
                  {orders.length === 0 ? (
                    ...
                    orders.map((order) => (
                      <tr key={order.id}>
                        ...
                        <td className="px-2 py-1 border-b">{new Date(order.created_at).toLocaleDateString()}</td>
