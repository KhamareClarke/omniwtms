...
const DeliveryMap = ({ stops }: { stops: DeliveryStop[] }) => {
  ...
  const validStops = stops.filter((stop: DeliveryStop) => 
  ...
  const positions: [number, number][] = validStops.map(stop => [stop.latitude, stop.longitude]);
      ...
      <div ref={labelRef} className="bg-white p-6 rounded-lg border">
        ...
        <div className="grid grid-cols-2 gap-4 mt-4">
          <div>
            ...
            <p className="text-sm">{delivery.delivery_stops?.find(stop => stop.stop_type === 'pickup')?.address}</p>
          </div>
          <div>
            <h4 className="font-medium">Delivery Location</h4>
            <p className="text-sm">{delivery.delivery_stops?.find(stop => stop.stop_type === 'delivery')?.address}</p>
          </div>
        </div>
              ...
              <div key={index} className="border rounded p-2">
                ...
                <div className="text-sm text-gray-500 space-y-1">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <p>Dimensions: {product.dimensions?.length ? `${product.dimensions.length} x ${product.dimensions.width} x ${product.dimensions.height} ${product.dimensions.unit}` : 'Not specified'}</p>
                    <p>Weight: {product.weight?.value ? `${product.weight.value} ${product.weight.unit}` : 'Not specified'}</p>
                  </div>
                </div>
              </div>
            ))}
...
export default function CourierContent() {
  ...
  const fetchCourierDeliveries = async (courierId: string) => {
    try {
      ...
      const deliveriesWithStops = (deliveriesData || []).map((delivery) => {
        ...
        const deliveryStops = routeInfo.stops?.map((stop: { name: string; distance: number; duration: number; traffic: string; weather: string; latitude?: number; longitude?: number }, index: number, stops: Array<{ name: string; distance: number; duration: number; traffic: string; weather: string }>) => {
  ...
  const handleStopUpdate = async (
    ...
    try {
      ...
      console.log('Updating stop:', { stops, stop, index });

      // Create new stop record with proper type assertions
      ...
      // Update deliveries state with proper type handling
      setDeliveries(prevDeliveries => 
        prevDeliveries.map(delivery => {
          if (delivery.id === stop.delivery_id && delivery.delivery_stops) {
            return {
              ...delivery,
              delivery_stops: delivery.delivery_stops.map(s => 
  ...
  const handleDeliveryUpdate = (updatedDelivery: Partial<Delivery> & { id: string }): void => {
    setDeliveries(prevDeliveries => 
      prevDeliveries.map(delivery => 
        delivery.id === updatedDelivery.id ? {
          ...delivery,
          ...updatedDelivery,
          status: (updatedDelivery.status || delivery.status) as Delivery['status'],
          delivery_stops: updatedDelivery.delivery_stops?.map(stop => ({
            ...stop,
                                    ...
                                    onClick={(e) => {
                                      ...
                                      if (delivery.delivery_stops) {
                                        handleStopUpdate(
                                          delivery.delivery_stops,
                                          stop,
                                          delivery.delivery_stops.findIndex(s => s.id === stop.id)
                                        );
                                      }
                                    }}
                                  >
      ...
      <main className="flex-1 p-6 overflow-y-auto bg-gradient-to-br from-blue-50 via-white to-blue-50">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <p className="text-sm text-gray-500">Your latest delivery assignments and updates</p>
          </div>
        ...
        <Dialog open={selectedDeliveryId !== null} onOpenChange={() => setSelectedDeliveryId(null)}>
